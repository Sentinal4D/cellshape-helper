...
def label_tif_to_pc_directory(
    path: str, save_dir: str, num_points: int, min_size: tuple = None
):
            ...
            if any(fpath.endswith(f) for f in acceptable_formats):
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    for prop in properties:
                        futures.append(
                            executor.submit(get_current_label_binary, prop)
                        )
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        if False not in valid:
                            ...
                            save_mesh_file = (
                                os.path.join(mesh_save_dir, mesh_file) + ".off"
                            )
                            save_point_cloud_file = (
                                os.path.join(point_cloud_save_dir, mesh_file)
                                + ".ply"
                            )
                            mesh_obj.export(save_mesh_file)
                            data = read_off(
                                os.path.join(mesh_save_dir, save_mesh_file)
                            )
                            points = sample_points(
                                data=data, num=num_points
                            ).numpy()
                            cloud = PyntCloud(
                                pd.DataFrame(
                                    data=points, columns=["x", "y", "z"]
                                )
                            )
